<template>
  ...
  <form @submit.prevent="submit" method="POST" class="max-w-md mx-auto mt-8">
      <div class="mb-6">
          <label for="name" class="block mb-2 uppercase font-bold text-xs test-gray-700">
              Name
          </label>

          <input v-model="form.name" type="text" class="border border-gray-400 p-2 w-full" name="name" id="name">
          <div v-if="form.errors.name" v-text="form.errors.name" class="text-red-500 text-xm mt-1"></div>
      </div>
      <div class="mb-6">
